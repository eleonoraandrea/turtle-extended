...
func (c *Client) do(ctx context.Context, method, path string, body interface{}, out interface{}) error {
	...
	if out != nil {
		...
		if !wrapper.Success {
			return fmt.Errorf("orderly !success %s", wrapper.Message)
		}
...
func (c *Client) PlaceOrder(ctx context.Context, req execution.OrderRequest) (execution.OrderResponse, error) {
	...
	if err := c.do(ctx, "POST", "/v1/order", payload, &resp); err != nil {
...
func (c *Client) GetPositions(ctx context.Context) ([]execution.Position, error) {
	...
	if err := c.do(ctx, "GET", "/v1/positions", nil, &data); err != nil {
		return nil, err
	}
...
func (c *Client) GetBalance(ctx context.Context) (execution.Balance, error) {
	...
	if err := c.do(ctx, "GET", "/v1/client/holding", nil, &data); err != nil {
		return execution.Balance{}, err
	}
	total := 0.0
	for _, h := range data.Holding {
		total += h.Holding
	}
...
func (c *Client) GetSymbols(ctx context.Context) ([]execution.SymbolInfo, error) {
	var data struct {
		Rows []struct {
			...
			MinNotional int     `json:"min_notional"`
		} `json:"rows"`
	}
	if err := c.do(ctx, "GET", "/v1/public/info", nil, &data); err != nil {
		return nil, err
	}
	var out []execution.SymbolInfo
	for _, r := range data.Rows {
		out = append(out, execution.SymbolInfo{Symbol: r.Symbol, QuoteTick: r.QuoteTick, BaseTick: r.BaseTick, MinNotional: float64(r.MinNotional)})
	}
	return out, nil
}
