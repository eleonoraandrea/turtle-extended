...
func Compute(res *backtest.Result) Stats {
	...
	for i, e := range res.Equity {
		equityVals[i] = e.Equity
	}
	rets := logReturns(equityVals)
	// Volatility annualized — use actual bars per year based on interval to avoid 252 trading-day bias for crypto 24/7
	// For 4h: 2190/year (8760/4), for 1h: 8760/year — crypto runs 365d, not 252
	barsPerDay := estimateBarsPerDay(res)
	annFactor := math.Sqrt(252 * barsPerDay)
	if barsPerDay == 0 {
		annFactor = math.Sqrt(252)
	}
	// Crypto correction: if interval suggests 365d market, prefer 365*barsPerDay; blend?
	// Keep backward compat but also compute true crypto factor for comparison, use crypto for 4h/1h
	if barsPerDay >= 6 { // intraday crypto: use 365 days
		annFactor = math.Sqrt(365 * barsPerDay)
	}
	mean := mean(rets)
	...
	if len(res.Trades) > 0 {
		...
		if wins > 0 {
			s.AvgWin = sumWin / float64(wins)
		} else {
			s.AvgWin = 0
		}
		if losses > 0 {
			s.AvgLoss = sumLoss / float64(losses)
		} else {
			s.AvgLoss = 0
		}
		if grossLoss != 0 {
		...
		if s.AvgLoss != 0 {
			s.PayoffRatio = s.AvgWin / math.Abs(s.AvgLoss)
		} else {
			s.PayoffRatio = math.NaN()
		}
	...
	if len(res.Bars) == len(res.Equity) {
		...
		for i, b := range res.Bars {
			bhEquity[i] = s.InitialCapital * b.Close / p0
		}
