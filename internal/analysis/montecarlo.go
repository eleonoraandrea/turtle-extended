...
func MonteCarlo(bars data.Bars, strat strategy.Strategy, cfg *config.Config, eng backtest.EngineConfig, runs int, perturbPct float64, blockSize int, seed int64) MCResult {
	...
	for i:=0;i<runs;i++{
		...
		for len(sample) < len(trades) {
			...
			blockLen:= blockSize
			if rnd.Float64() < 0.3 { blockLen = 1 + rnd.Intn(blockSize) } // variable
		...
		var rets []float64
		for _,t:=range sample{rets=append(rets, t.PnLNet)}
		sh:= tradeSharpe(rets)
	...
	probProfit:=0.0
	for _,r:=range returns{if r>0{probProfit++}}
	if len(returns)>0{probProfit = probProfit / float64(len(returns)) * 100}
...
func meanRets(a []float64) float64 { s:=0.0; for _,v:=range a{s+=v}; return s/float64(len(a)) }
func stdRets(a []float64, m float64) float64 { s:=0.0; for _,v:=range a{d:=v-m; s+=d*d}; return math.Sqrt(s/float64(len(a)-1))}
