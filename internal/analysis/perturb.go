...
func Perturb(bars data.Bars, baseCfg *config.Config, variant, symbol string, eng backtest.EngineConfig) []PerturbResult {
	...
	perturbations := []struct{
		...
		{"donchian_entry", func(c *config.Config){ c.VariantA.DonchianEntry = int(float64(c.VariantA.DonchianEntry)*1.2); c.VariantB.DonchianEntry = int(float64(c.VariantB.DonchianEntry)*1.2); c.VariantD.DonchianFast = int(float64(c.VariantD.DonchianFast)*1.2) }, 55, 11},
		{"donchian_entry -20%", func(c *config.Config){ c.VariantA.DonchianEntry = int(float64(c.VariantA.DonchianEntry)*0.8); c.VariantB.DonchianEntry = int(float64(c.VariantB.DonchianEntry)*0.8); c.VariantD.DonchianFast = int(float64(c.VariantD.DonchianFast)*0.8) }, 55, -11},
		{"atr_stop +20%", func(c *config.Config){ c.VariantA.ATRStopMult *= 1.2; c.VariantB.ATRStopMult *= 1.2; c.VariantD.ATRStopMult *= 1.2; c.ATRConf.InitialStop *=1.2 }, 1.8, 0.36},
		{"atr_stop -20%", func(c *config.Config){ c.VariantA.ATRStopMult *= 0.8; c.VariantB.ATRStopMult *= 0.8; c.VariantD.ATRStopMult *= 0.8; c.ATRConf.InitialStop *=0.8 }, 1.8, -0.36},
		{"risk base +0.5%", func(c *config.Config){ c.Risk.Base += 0.005; c.Risk.Max += 0.005 }, 0.01, 0.005},
		{"risk base -0.5%", func(c *config.Config){ c.Risk.Base -= 0.005; if c.Risk.Base<0.0025 {c.Risk.Base=0.0025}; c.Risk.Max -=0.005 }, 0.01, -0.005},
	}
...
func PerturbSummary(results []PerturbResult) string {
	if len(results)==0{return "no data"}
	base := results[0]
	var sb string
	...
	for _,r:=range results[1:]{
		if r.ReturnPct<minRet{minRet=r.ReturnPct}
		if r.ReturnPct>maxRet{maxRet=r.ReturnPct}
