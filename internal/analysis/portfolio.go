...
func RunPortfolio(barsMap map[string]data.Bars, cfg *config.Config, variant, interval string) *PortfolioResult {
	...
	for sym, bars := range barsMap {
		...
		if variant!="D" { eng.TrailMode="donchian" }
	...
	for _, b := range barsMap { refBars = b; break }
	...
	if len(combinedEquity)>0 {
		...
		if factor!=1 && !math.IsNaN(factor) && !math.IsInf(factor,0) {
			for i:=range combinedEquity { combinedEquity[i].Equity *= factor }
		}
	}
	...
	for _, r := range perSymbol {
		...
		if r.MaxHeatSeen > synthRes.MaxHeatSeen { synthRes.MaxHeatSeen = r.MaxHeatSeen }
	}
	combinedStats := metrics.Compute(synthRes)

	// symbols list
	var syms []string
	for k:= range barsMap { syms = append(syms, k) }
...
// Helper to load barsMap for portfolio
