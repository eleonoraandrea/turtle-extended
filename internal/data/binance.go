...
func NewBinanceClient(base string) *BinanceClient {
	if base == "" { base = DefaultBinanceBase }
...
func (c *BinanceClient) FetchKlines(symbol, interval string, startTime, endTime time.Time) (Bars, error) {
	...
	for {
		if !endTime.IsZero() && !cur.Before(endTime) { break }
		...
		if !last.After(cur) { break }
		cur = last
		if !endTime.IsZero() && (cur.After(endTime) || cur.Equal(endTime)) { break }
		time.Sleep(120 * time.Millisecond)
		if len(all) > 200000 { break }
	}
	return all, nil
}
...
func (c *BinanceClient) fetchKlinesChunk(symbol, interval string, start, end time.Time) (Bars, error) {
	...
	if !start.IsZero() { u += fmt.Sprintf("&startTime=%d", start.UnixMilli()) }
	if !end.IsZero() { u += fmt.Sprintf("&endTime=%d", end.UnixMilli()) }
	resp, err := c.HTTP.Get(u)
	if err != nil { return nil, fmt.Errorf("binance klines GET %w", err)}
...
func (c *BinanceClient) FetchFundingRate(symbol string, start, end time.Time, limit int) ([]FundingRate, error) {
	...
	if !start.IsZero() { u += fmt.Sprintf("&startTime=%d", start.UnixMilli()) }
	if !end.IsZero() { u += fmt.Sprintf("&endTime=%d", end.UnixMilli()) }
	// paginate if range large: Binance returns most recent, need loop backwards
	var all []FundingRate
	curStart := start
	for {
		uu := u
		if !curStart.IsZero() { /* already */ }
		resp, err := c.HTTP.Get(uu)
		if err != nil { return nil, err}
		b, _:= io.ReadAll(resp.Body)
		resp.Body.Close()
		if resp.StatusCode!=200 { return nil, fmt.Errorf("funding %d %s", resp.StatusCode, string(b)) }
		var chunk []FundingRate
		// funding rate as string decode
		var raw []struct{
			Symbol string `json:"symbol"`
		...
		// advance start after last fundingTime
		last := chunk[len(chunk)-1].FundingTime
		curStart = time.UnixMilli(last+1)
		u = fmt.Sprintf("%s/fapi/v1/fundingRate?symbol=%s&limit=%d&startTime=%d", c.Base, url.QueryEscape(symbol), limit, curStart.UnixMilli())
		if !end.IsZero() && curStart.After(end) { break }
		if len(all) > 10000 { break }
		if len(chunk) < limit { break }
		// avoid infinite; break after one if start empty? Need check start zero handling.
		if start.IsZero() && end.IsZero() { break }
		// Binance funding pagination quirk: if we loop with startTime we need endTime
		if !end.IsZero() { u += fmt.Sprintf("&endTime=%d", end.UnixMilli()) }
...
func (c *BinanceClient) FetchOpenInterestHist(symbol, period string, start, end time.Time, limit int) ([]OIHist, error) {
	...
	defer resp.Body.Close()
	b,_:=io.ReadAll(resp.Body)
	if resp.StatusCode!=200{return nil, fmt.Errorf("oiHist %d %s", resp.StatusCode, string(b))}
	// response shape: [{symbol, sumOpenInterest, sumOpenInterestValue, timestamp}]
	var raw []map[string]interface{}
	if err:=json.Unmarshal(b,&raw);err!=nil{return nil, fmt.Errorf("oi json %w body %s", err, string(b))}
	var out []OIHist
	for _,m:=range raw{
		s,_:=m["symbol"].(string)
		ts:=toInt64(m["timestamp"])
		// sumOpenInterest may be string
		var soi float64
		if v,ok:=m["sumOpenInterest"];ok{soi=toFloat(v)}
		if soi==0 { if v,ok:=m["sumOpenInterestValue"]; ok { soi=toFloat(v)}}
		out=append(out, OIHist{Symbol:s, SumOpenInterest:soi, Timestamp:ts})
	}
	return out,nil
}
...
func LoadBarsCSV(path string) (Bars, error) {
	...
	for {
		...
		vol:=0.0; if len(rec)>5{vol,_=strconv.ParseFloat(rec[5],64)}
		qv:=0.0; if len(rec)>6{qv,_=strconv.ParseFloat(rec[6],64)}
		tr:=int64(0); if len(rec)>7{tr,_=strconv.ParseInt(rec[7],10,64)}
		tb:=0.0; if len(rec)>8{tb,_=strconv.ParseFloat(rec[8],64)}
		fr:=0.0; if len(rec)>9{fr,_=strconv.ParseFloat(rec[9],64)}
		oi:=0.0; if len(rec)>10{oi,_=strconv.ParseFloat(rec[10],64)}
		soi:=0.0; if len(rec)>11{soi,_=strconv.ParseFloat(rec[11],64)}
		mp:=0.0; if len(rec)>12{mp,_=strconv.ParseFloat(rec[12],64)}
...
func AlignDerivatives(bars Bars, fundings []FundingRate, ois []OIHist) Bars {
	...
	for i, bar := range out {
		...
		if lastMP!=0 { out[i].MarkPrice = lastMP }
	}
	return out
}
...
func toFloat(v interface{}) float64 {
	...
	s:=fmt.Sprintf("%v", v)
	if f,err:=strconv.ParseFloat(s,64);err==nil && !math.IsNaN(f) && !math.IsInf(f,0){return f}
	return 0
}
func toInt64(v interface{}) int64 {
	...
	if f,err:=strconv.ParseFloat(s,64);err==nil{return int64(f)}
	return 0
}
