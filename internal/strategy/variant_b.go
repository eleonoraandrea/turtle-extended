...
func (s *VariantB) Next(ctx *Context, i int) Signal {
	...
	// Vol regime veto extremes? For B we avoid high vol >80
	if !math.IsNaN(volReg) && volReg > c.VolHighPct {
		// still allow but we skip? Keep filter light
		// return 0 if high vol and ADX weak
	}
