...
func PrepareCommon(bars data.Bars, cfg *config.Config, variant string) *Context {
	n:=len(bars)
	closeP:=make([]float64,n); high:=make([]float64,n); low:=make([]float64,n); vol:=make([]float64,n)
	oi:=make([]float64,n); funding:=make([]float64,n)
	for i,b:=range bars{closeP[i]=b.Close; high[i]=b.High; low[i]=b.Low; vol[i]=b.Volume; oi[i]=b.SumOpenInterest; funding[i]=b.FundingRate}
	// pick ATR period depending variant
	atrPeriod:=20
	if variant=="D"{atrPeriod=cfg.VariantD.ATRPeriod} else if variant=="C"{atrPeriod=cfg.VariantC.ATRPeriod} else if variant=="B"{atrPeriod=cfg.VariantB.ATRPeriod} else {atrPeriod=cfg.VariantA.ATRPeriod}
...
func volConfirm(vol, volSMA float64, mult float64) bool {
	if isNaN(volSMA) || volSMA==0 {return true} // warmup pass
