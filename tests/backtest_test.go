...
func TestIndicatorsWarmupNaN(t *testing.T) {
	...
	if ctx.ATR[5] == ctx.ATR[5] { /* should be NaN for early */
	} // first 20 warmup is NaN
	// check warmup filters signal 0
	sig := strat.Next(ctx, 5)
	if sig.Side != 0 {
		t.Fatalf("expected warmup 0 got %d %s", sig.Side, sig.Reason)
	}
}

func TestRiskSizing(t *testing.T) {
	cfg, _ := config.Load("../configs/default.yaml")
	if cfg.VariantA.RiskPct != 2.0 {
		t.Fatalf("config A risk %f", cfg.VariantA.RiskPct)
	}
...
func TestCSVRoundTrip(t *testing.T) {
	...
	if err := data.SaveBarsCSV(path, bars); err != nil {
		t.Fatal(err)
	}
	...
	if len(loaded) != len(bars) {
		t.Fatalf("len mismatch %d vs %d", len(loaded), len(bars))
	}
	diff := loaded[0].Close - bars[0].Close
	if diff > 1e-6 || diff < -1e-6 {
		t.Fatalf("close mismatch %.8f vs %.8f diff %.10f", loaded[0].Close, bars[0].Close, diff)
	}
}
...
func TestRiskSizingHonorsMaxRisk(t *testing.T) {
	...
	if !dec.Accept {
		t.Fatalf("rejected: %v", dec.Factors)
	}
	if math.Abs(dec.RiskPct-2.0) > 0.01 {
		t.Fatalf("risk pct %.4f != 2.0", dec.RiskPct)
	}
	if math.Abs(dec.Qty-2.0) > 0.0001 {
		t.Fatalf("qty %.4f != 2.0", dec.Qty)
	}
	if math.Abs(dec.RiskAmount-200) > 0.5 {
		t.Fatalf("risk amount %.2f != 200", dec.RiskAmount)
	}
	// notional 4000 on 10k → leverage 0.4
	if dec.Leverage > 1.0 {
		t.Fatalf("leverage %.2f unexpectedly high", dec.Leverage)
	}
}
...
func TestRiskDynamicLeverageCap(t *testing.T) {
	...
	if dec.Leverage > 7.0+1e-6 {
		t.Fatalf("leverage %.2f exceeds dynamic cap 7.0 (high vol regime ×0.70)", dec.Leverage)
	}
	if dec.RiskPct > 2.0+0.01 {
		t.Fatalf("risk %.3f exceeds 2%%", dec.RiskPct)
	}
	// weak ADX → ×0.80 → 8.0x
	ms2 := risk.MarketState{Equity: 10000, Price: 2000, ATR: 5, StopPrice: 1999, Side: 1, VolRegime: 50, ADX: 10}
	dec2 := risk.Size(ms2, lim)
	if dec2.Leverage > 8.0+1e-6 {
		t.Fatalf("leverage %.2f exceeds dyn cap 8.0 (weak ADX ×0.80)", dec2.Leverage)
	}
	...
	if dec3.Leverage > 10.0+1e-6 {
		t.Fatalf("leverage %.2f exceeds HARD cap 10.0", dec3.Leverage)
	}
}
...
func TestRiskHeatBudgetRejects(t *testing.T) {
	...
	dec := risk.Size(ms, lim)
	if !dec.Accept {
		t.Fatalf("should accept with remaining heat")
	}
	if dec.RiskPct > 0.1+0.05 {
		t.Fatalf("risk %.3f should be clipped to ~0.1%% remaining heat (8-7.9)", dec.RiskPct)
	}
	ms2 := risk.MarketState{Equity: 10000, Price: 2000, ATR: 50, StopPrice: 1900, Side: 1, PortfolioHeatPct: 8.5}
	dec2 := risk.Size(ms2, lim)
	if dec2.Accept {
		t.Fatalf("should REJECT when heat exhausted (8.5>8)")
	}
}
...
func TestRiskDrawdownDeleverage(t *testing.T) {
	...
	if math.Abs(dec.RiskPct-0.625) > 0.05 {
		t.Fatalf("dd adaptive risk %.3f != ~0.625%%", dec.RiskPct)
	}
	...
	dec2 := risk.Size(ms2, lim)
	if !dec2.Accept {
		t.Fatalf("should keep min risk in flat zone (adaptive)")
	}
	if math.Abs(dec2.RiskPct-0.25) > 0.02 {
		t.Fatalf("dd flat risk %.3f != min 0.25%%", dec2.RiskPct)
	}
}
...
func TestVolTargetScalesRisk(t *testing.T) {
	...
	if dec.RiskPct > 1.35 || dec.RiskPct < 1.15 {
		t.Fatalf("vol target not applied: risk %.3f should be ~1.25%% (50/80)", dec.RiskPct)
	}
}

func TestBacktestRiskInvariants(t *testing.T) {
	cfg, _ := config.Load("../configs/default.yaml")
	if err := risk.ValidateLimitInvariants(risk.LimitsFromConfig(cfg, "A")); err != nil {
...
func TestStopLossEqualsRiskBudget(t *testing.T) {
	...
	stops := 0
	for _, tr := range res.Trades {
		if tr.ExitReason != "stop" || tr.StopDist <= 0 {
			continue
		}
		stops++
	...
	if stops == 0 {
		t.Skip("no stop trades in sample")
	}
