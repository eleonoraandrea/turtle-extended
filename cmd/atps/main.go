...
func loadCfg() *config.Config{
	...
	if err!=nil{
		...
		if err2!=nil{ fmt.Fprintf(os.Stderr,"config load failed %v\n",err); os.Exit(1)}
		return c2
	}
	return c
}
...
func cmdDownload() *cobra.Command{
	...
	cmd:= &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string){
			...
			if base==""{base=data.DefaultBinanceBase}
			client:= data.NewBinanceClient(base)
			if symbol==""{symbol=cfg.General.Symbols[0]}
			if interval==""{interval=cfg.General.Interval}
			if out==""{out=fmt.Sprintf("data/raw/%s_%s.csv", symbol, interval)}
			st:= parseTime(start, cfg.General.StartTime)
			et:= parseTime(end, cfg.General.EndTime)
			if et.IsZero(){et=time.Now().UTC()}
			...
			if err!=nil{fmt.Fprintf(os.Stderr,"klines error %v\n",err); os.Exit(1)}
			fmt.Printf("klines %d\n", len(bars))
			var fundings []data.FundingRate
			var ois []data.OIHist
			if withFunding{
				f,err:= client.FetchFundingRate(symbol, st, et, 1000)
				if err!=nil{fmt.Printf("funding warn %v\n",err)}else{fundings=f; fmt.Printf("funding %d\n",len(fundings))}
			}
			if withOI{
				oiPeriod:= cfg.Data.OIPeriod
				if oiPeriod==""{oiPeriod="4h"}
				o,err:= client.FetchOpenInterestHist(symbol, oiPeriod, st, et, 500)
				if err!=nil{fmt.Printf("oi warn %v\n",err)}else{ois=o; fmt.Printf("oi %d\n",len(ois))}
				...
				if len(ois)==500{
					...
					for {
						...
						if err!=nil||len(more)==0{break}
						ois=append(ois, more...)
						cur=more[len(more)-1].Timestamp
						if len(more)<500{break}
						fmt.Printf("oi paginated %d total\n",len(ois))
						if len(ois)>5000{break}
					}
				}
			}
			...
			if err:= data.SaveBarsCSV(out, bars);err!=nil{fmt.Fprintf(os.Stderr,"save %v\n",err); os.Exit(1)}
	...
	cmd:= &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string){
			...
			if csvPath==""{
				...
				if interval==""{interval=cfg.General.Interval; csvPath=fmt.Sprintf("data/raw/%s_%s.csv", symbol, interval)}
			}
			var bars data.Bars
			var err error
			if _,e:=os.Stat(csvPath); e==nil{
				bars,err= data.LoadBarsCSV(csvPath)
				if err!=nil{fmt.Fprintf(os.Stderr,"load csv %v\n",err); os.Exit(1)}
			...
			if outHTML==""{ outHTML=fmt.Sprintf("reports/%s_%s_%s.html", symbol, variant, time.Now().Format("20060102_1504"))}
			in:= report.Input{Config:cfg, Bars:bars, Result:res, Stats:stats, Symbol:symbol, Variant:variant, GeneratedAt: time.Now()}
			if err:= report.Generate(outHTML, in); err!=nil{fmt.Fprintf(os.Stderr,"report %v\n",err); os.Exit(1)}
			fmt.Printf("report %s\n", outHTML)
			// save trades json
...
func cmdCompare() *cobra.Command{
	...
	cmd:= &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string){
			...
			if symbolsCSV==""{syms=cfg.Compare.Symbols}
			if len(syms)==0|| (len(syms)==1 && syms[0]==""){syms=cfg.General.Symbols}
			vars:= strings.Split(variantsCSV,",")
			if variantsCSV==""{vars=cfg.Compare.Variants}
			if len(vars)==0||vars[0]==""{vars=[]string{"A","B","C","D"}}
			...
			for _,sym:=range syms{
				...
				if _,e:=os.Stat(csvPath); e==nil{
					...
					if seed==0{seed= 42}
					for _,c:=range sym{seed+=int64(c)*3}
			...
			if out==""{out="reports/comparison.html"}
			if err:= report.GenerateComparison(out, rows, cfg); err!=nil{fmt.Fprintf(os.Stderr,"cmp report %v\n",err); os.Exit(1)}
...
func cmdWalkForward() *cobra.Command{
	...
	cmd:= &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string){
			...
			if variant==""{variant="D"}
			variant=strings.ToUpper(variant)
			if folds==0{folds=cfg.WalkForward.Folds}
			if csvPath==""{csvPath=fmt.Sprintf("data/raw/%s_%s.csv", symbol, cfg.General.Interval)}
			var bars data.Bars
			if _,e:=os.Stat(csvPath);e==nil{b,_:=data.LoadBarsCSV(csvPath); bars=b}else{
				seedMap:= map[string]int64{"BTCUSDT":42,"ETHUSDT":1337,"SOLUSDT":9999}
				seed:= seedMap[symbol]
				if seed==0{seed=42}
				bars=data.GenerateSynthetic(3500, 4*time.Hour, seed)
			...
			if out==""{out=fmt.Sprintf("reports/%s_%s_WF.json", symbol, variant)}
...
func cmdMonteCarlo() *cobra.Command{
	...
	cmd:= &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string){
			...
			if variant==""{variant="D"}
			variant=strings.ToUpper(variant)
			if runs==0{runs=cfg.MonteCarlo.Runs}
			if csvPath==""{csvPath=fmt.Sprintf("data/raw/%s_%s.csv", symbol, cfg.General.Interval)}
			var bars data.Bars
			if _,e:=os.Stat(csvPath);e==nil{b,_:=data.LoadBarsCSV(csvPath); bars=b}else{
				seedMap:= map[string]int64{"BTCUSDT":42,"ETHUSDT":1337,"SOLUSDT":9999}
			...
			if out==""{out=fmt.Sprintf("reports/%s_%s_MC.json", symbol, variant)}
			os.MkdirAll(filepath.Dir(out),0755)
...
func cmdPerturb() *cobra.Command{
	...
	cmd:= &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string){
			...
			if out==""{out=fmt.Sprintf("reports/%s_%s_PERTURB.json", symbol, variant)}
			os.MkdirAll(filepath.Dir(out),0755)
...
func cmdPortfolio() *cobra.Command{
	...
	cmd:= &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string){
			...
			if out==""{out=fmt.Sprintf("reports/PORTFOLIO_%s.html", variant)}
			...
			var rows []report.ComparisonRow
			for sym, st := range pr.PerSymbolStats { rows=append(rows, report.ComparisonRow{Symbol:sym, Variant:variant, Stats:st}) }
...
func engineFromCfg(cfg *config.Config, variant, symbol string) backtest.EngineConfig{
	trailMode:= "donchian"
	if variant=="D"|| variant=="d"{ trailMode=cfg.VariantD.TrailMode } else { trailMode="donchian"}
...
func parseTime(s, fallback string) time.Time{
	...
	for _,l:=range layouts{
		if t,err:= time.Parse(l, s);err==nil{return t.UTC()}
	}
	// try fallback
	if t,err:= time.Parse("2006-01-02", s);err==nil{return t.UTC()}
	return time.Time{}
}
